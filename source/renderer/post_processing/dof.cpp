...
DoF::DoF(Renderer* pRenderer) : m_pRenderer(pRenderer)
{
    ...
    psoDesc.cs = pRenderer->GetShader("dof/downsample.hlsl", "main", "cs_6_6", {});
    m_pDownsamplePSO = pRenderer->GetPipelineState(psoDesc, "DOF - downsample PSO");

    psoDesc.cs = pRenderer->GetShader("dof/circular_filter_far.hlsl", "main_horizontal", "cs_6_6", {});
    m_pFarHorizontalBlurPSO = pRenderer->GetPipelineState(psoDesc, "DOF - far blur X PSO");

    psoDesc.cs = pRenderer->GetShader("dof/circular_filter_far.hlsl", "main_vertical", "cs_6_6", {});
    m_pFarVerticalBlurPSO = pRenderer->GetPipelineState(psoDesc, "DOF - far blur Y PSO");

    psoDesc.cs = pRenderer->GetShader("dof/tile_coc.hlsl", "main", "cs_6_6", {});
    m_pTileCocXPSO = pRenderer->GetPipelineState(psoDesc, "DOF - tile coc dilate PSO");

    psoDesc.cs = pRenderer->GetShader("dof/tile_coc.hlsl", "main", "cs_6_6", { "VERTICAL_PASS=1" });
    m_pTileCocYPSO = pRenderer->GetPipelineState(psoDesc, "DOF - tile coc dilate PSO");

    psoDesc.cs = pRenderer->GetShader("dof/circular_filter_near.hlsl", "main_horizontal", "cs_6_6", {});
    m_pNearHorizontalBlurPSO = pRenderer->GetPipelineState(psoDesc, "DOF - near blur X PSO");

    psoDesc.cs = pRenderer->GetShader("dof/circular_filter_near.hlsl", "main_vertical", "cs_6_6", {});
    m_pNearVerticalBlurPSO = pRenderer->GetPipelineState(psoDesc, "DOF - near blur Y PSO");

    psoDesc.cs = pRenderer->GetShader("dof/composite.hlsl", "main", "cs_6_6", {});
