...
DLSS::DLSS(Renderer* pRenderer)
{
    ...
    m_pDlssInitCommandList.reset(pRenderer->GetDevice()->CreateCommandList(GfxCommandQueue::Compute, "DLSS::m_pDlssInitCommandList"));
    ...
    RenderGraphHandle exposure, uint32_t displayWidth, uint32_t displayHeight)
{
    ...
    GUI("PostProcess", "DLSS", [&]()
        {
            ...
            {
                m_pRenderer->SetTemporalUpscaleRatio(1.5f); //todo
            }
        });
    ...
    auto dlss_pass = pRenderGraph->AddPass<DLSSPassData>("DLSS", RenderPassType::Compute,
        ...
        [=](const DLSSPassData& data, IGfxCommandList* pCommandList)
        {
            ...
            dlssEvalParams.Feature.InSharpness = 0.35f;
...
bool DLSS::InitializeDLSSFeatures(uint32_t displayWidth, uint32_t displayHeight)
{
    ...
    NVSDK_NGX_PerfQuality_Value qualityMode = NVSDK_NGX_PerfQuality_Value_MaxQuality;
    ...
    dlssCreateParams.Feature.InWidth = optimalWidth;
    dlssCreateParams.Feature.InHeight = optimalHeight;
