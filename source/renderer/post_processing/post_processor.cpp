...
RenderGraphHandle PostProcessor::Process(RenderGraph* pRenderGraph, const PostProcessInput& input, uint32_t width, uint32_t height)
{
    ...
    RenderGraphHandle outputHandle;
    
    outputHandle = m_pTAA->Render(pRenderGraph, input.sceneColorRT, input.sceneDepthRT, input.linearDepthRT, input.velocityRT, width, height);
