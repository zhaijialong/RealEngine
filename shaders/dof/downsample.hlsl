...
void main(uint2 dispatchThreadID : SV_DispatchThreadID)
{
    ...
    float coc00 = ComputeCoc(depth00, apertureWidth, focalLength, c_focusDistance, sensorWidth);
    float coc10 = ComputeCoc(depth10, apertureWidth, focalLength, c_focusDistance, sensorWidth);
    float coc01 = ComputeCoc(depth01, apertureWidth, focalLength, c_focusDistance, sensorWidth);
    float coc11 = ComputeCoc(depth11, apertureWidth, focalLength, c_focusDistance, sensorWidth);
    ...
    float4 near = float4(color00, 1.0) * max(-coc00, 0.0f) +
        float4(color10, 1.0) * max(-coc10, 0.0f) +
        float4(color01, 1.0) * max(-coc01, 0.0f) +
        float4(color11, 1.0) * max(-coc11, 0.0f);
